...
class PiCameraTrackWithMQTT(VideoStreamTrack):
    ...
    def get_system_stats(self):
        ...
        # Temperatura (specific Raspberry Pi)
        try:
        ...
        # Procentaj Baterie (folosind clasa UPS definită anterior)
        bat = 0.0
    ...
    async def recv(self):
        ...
        # Publish realtime sensor data to MQTT (unghi + viteza only)
        if self.mqtt_handler and self.mqtt_handler.connected:
        ...
        cv2.putText(frame, f"Timestamp: {timestamp_ms}", (15, 40), font, 1.0, (0, 255, 255), 3)

        # Frame info
        cv2.putText(frame, f"Frame: {self.frame_count} | Time: {elapsed:.2f}s", (15, 80), font, 0.8, (0, 255, 255), 2)

        # Sensor data
        cv2.putText(frame, f"Unghi: {unghi:.2f}deg | Viteza: {viteza:.1f} RPM", (15, 120), font, 0.8, (0, 255, 255), 2)

        # Mode and commands
        mode_text = "AUTO" if mode == 0 else "MANUAL"
        cv2.putText(frame, f"Mode: {mode_text} | Cmd: {unghi:.0f}deg, {viteza:.0f}RPM", (15, 160), font, 0.8, (255, 0, 255), 2)
