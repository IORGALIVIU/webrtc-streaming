...
class VideoReceiverGUI_MQTT:
    ...
    def _setup_ui(self):
        ...
        # Angle control
        ttk.Label(control_panel, text="Manual Angle:").grid(row=1, column=0, sticky=tk.W, padx=10, pady=5)
        angle_scale = ttk.Scale(control_panel, from_=-180, to=180, variable=self.control_angle,
                               orient=tk.HORIZONTAL, length=200, command=self._on_angle_changed)
        angle_scale.grid(row=1, column=1, columnspan=2, padx=5)
        self.angle_value_label = ttk.Label(control_panel, text="0°")
        self.angle_value_label.grid(row=2, column=1, columnspan=2)
        
        # Speed control
        ttk.Label(control_panel, text="Manual Speed:").grid(row=3, column=0, sticky=tk.W, padx=10, pady=5)
        speed_scale = ttk.Scale(control_panel, from_=0, to=100, variable=self.control_speed,
                               orient=tk.HORIZONTAL, length=200, command=self._on_speed_changed)
        speed_scale.grid(row=3, column=1, columnspan=2, padx=5)
        self.speed_value_label = ttk.Label(control_panel, text="0 RPM")
        self.speed_value_label.grid(row=4, column=1, columnspan=2)
        
        # Send button
        ttk.Button(control_panel, text="Send Commands", command=self._send_all_commands).grid(
            row=5, column=0, columnspan=3, pady=10)
        
        # Last command sent display
        ttk.Label(control_panel, text="Last Sent:").grid(row=6, column=0, sticky=tk.W, padx=10, pady=5)
        self.last_command_label = ttk.Label(control_panel, text="None", foreground="green", wraplength=200)
        self.last_command_label.grid(row=6, column=1, columnspan=2, sticky=tk.W, padx=5)
    ...
    def _on_mode_changed(self):
        ...
        self.angle_value_label.config(text=f"{self.control_angle.get():.0f}°")
    
    def _on_angle_changed(self, value):
        """Angle slider changed."""
        self.angle_value_label.config(text=f"{float(value):.0f}°")
    
    def _on_speed_changed(self, value):
        """Speed slider changed."""
        self.speed_value_label.config(text=f"{float(value):.0f} RPM")
